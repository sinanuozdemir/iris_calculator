...
thread = Table('thread', post_meta,
    ...
    Column('tag', String(length=128)),
    Column('unique_thread_id', String(length=128)),
    Column('all_parties_replied', Boolean, default=ColumnDefault(False)),
    Column('latracking_reply', Boolean, default=ColumnDefault(False)),
    Column('auto_tag', Boolean, default=ColumnDefault(False)),
    Column('already_tagged', Boolean, default=ColumnDefault(False)),
...
def upgrade(migrate_engine):
    ...
    post_meta.tables['thread'].columns['tag'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    post_meta.tables['thread'].columns['tag'].drop()
